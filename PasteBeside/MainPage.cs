	...
	private static AutoLayout AvailableHandshakes(AvailableHandshakeViewModel viewModel) => new AutoLayout()
		...
		.Children(
			...
			new Viewbox()
				...
				.Height(15)
				.Child(new PathIcon()
					// NB: tempting to pass a Func<Brush> delegate into a helper method for the Viewbox, but
					// it doesn't work. Uno doesn't recognize that transitive binding to the viewModel.
